...
export default function FadeButton({styleMode, linkPath="/", width="w-full", height="h-full", extraClass="", children} : any)
{
    ...
    return (
        <Link to={linkPath} ref={ref1} className={`transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'} ${extraClass}`}>
            <button className={`group/fade-button relative ${width} ${height} rounded-lg ${bgStyle} hover:font-medium hover:scale-[102%] transition-all duration-150 overflow-hidden pointer-events-auto`}>
